    ...
    <div className="h-screen mx-auto p-4 lg:px-8 mb-8">
      ...
      {orders.length > 0 ? (
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <div className="text-sm sm:text-base">
                ...
                <p className="text-gray-600 hidden sm:block">We will contact you before delivery.</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center h-[450px] text-center px-4">
          ...
          />
          <h1 className="text-lg sm:text-xl font-semibold text-gray-800">You haven't placed any orders yet.</h1>
          <p className="text-gray-500 mb-4 text-sm sm:text-base">Discover amazing products and start shopping today!</p>
          <button
          ...
          >
            Shop Now
            <span className="material-symbols-outlined ml-2 text-lg sm:text-xl">shopping_cart</span>
          </button>
        </div>
      )}
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <p className="text-gray-700 font-medium">Order #{selectedOrder.id}</p>
            <p className="text-gray-600 mb-2">Total: ₹{selectedOrder.totalPrice}.00</p>
            <p className="text-gray-600 mb-2">Delivery: 5-7 days</p>
            <ul className="border-t pt-2 mb-4">
              {selectedOrder.items.map((item) => (
                <li key={item.id} className="text-gray-700 flex items-center space-x-2">
                  <span>{item.name} × {item.quantity} - ₹{item.price * item.quantity}.00</span>
                </li>
              ))}
            </ul>
      ...
      {showCancelPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center px-4">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <p className="text-gray-600 mb-2">Please provide a reason for cancellation:</p>
            <textarea
